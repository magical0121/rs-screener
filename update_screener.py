...
判定（インジ Lookahead ON / フィルターOFF 相当）:
  ...
  2. 週足終値 > 30週SMA
  → Stage2 条件
  → 昨日は未点灯 & 今日点灯 = Stage2 入り（Entry）※Lookahead ON日次
...
def calc_stage2_entry(close: pd.Series, high: pd.Series) -> dict:
    ...
    30週SMA・ブレイク・Stage維持はPineと同じ。

    Entry（背景が出た瞬間）:
      Lookahead ONの日足では、未確定週の終値が毎日更新される。
      → 今日までのデータで stage==2
      → 昨日までのデータでは stage!=2
      のときだけ Entry（＝今日点灯）

    ※「今週のどこかで入った銘柄を一週間中Entry表示」にはしない。
    """
    out = {
        "stage2_active": False,
        "stage2_entry": False,
    ...
    try:
        ...
        w_today = _build_weekly_stage(close, high)
        if w_today is None:
            return out

        w_yest = _build_weekly_stage(close.iloc[:-1], high.iloc[:-1])
        stage_today = int(w_today["stage"].iloc[-1])
        stage_yest = int(w_yest["stage"].iloc[-1]) if w_yest is not None and len(w_yest) else 1

        active = stage_today == 2
        entry = stage_today == 2 and stage_yest != 2
...
def main() -> None:
    ...
    for i in range(0, total, BATCH_SIZE):
        ...
        for t, df in hist.items():
            try:
                ...
                # 背景が入った瞬間のみ（継続中 Active は出さない）
                if not st["stage2_entry"]:
                    continue
    ...
    # RS 順
    stocks_sorted = sorted(stocks, key=lambda x: -(x.get("rs") or 0))
