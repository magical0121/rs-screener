...
def compute_industry_scores(stocks: list[dict], min_count: int = 3) -> list[dict]:
    ...
    for name, rows in groups.items():
        if len(rows) < min_count:
            continue
        # HQMが無いEntryモードではRS中心
        score = int(round(np.mean([
            (0.6 * float(s.get("rs") or 0) + 0.4 * float(s.get("hqm") or 0))
            if s.get("hqm") is not None else float(s.get("rs") or 0)
            for s in rows
        ])))
        if score >= 80:
            label = "強い"
...
def main() -> None:
    """本日の本 Stage2 Entry + 業種フル付与（FinanceDatabase → Finviz）"""
    themes = load_themes()
    ...
    for i in range(0, total, BATCH_SIZE):
        ...
        for t, df in hist.items():
            ...
            if not industry or industry in {"—", "-", "N/A", "n/a", "nan"}:
    ...
    payload = {
        ...
        "filters": {
            ...
            "range_len_weeks": RANGE_LEN_WEEKS,
            "sma_weeks": SMA_WEEKS,
        },
