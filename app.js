// 本日の本 Stage2 Entry スクリーナー
// 旧デザイン + 業種日本語 + 業種強度 / 業種スコア
...
const INDUSTRY_JA = {
  ...
  "Construction Materials": "建設資材",
  "Health Care Equipment & Supplies": "医療機器・用品",
  "Technology Hardware, Storage & Peripherals": "テックハード・ストレージ",
  "Real Estate Services": "不動産サービス",
  "Diversified Financial Services": "多角金融",
  "Farm & Heavy Construction Machinery": "農機・建機",
  "Other Industrial Metals & Mining": "その他工業金属・鉱業",
  "Other Precious Metals & Mining": "その他貴金属・鉱業",
  "Information Technology Services": "ITサービス",
...
function industryJa(name) {
  if (!name || name === "—" || name === "-" || name === "nan" || name === "N/A") return "—";
  const k = String(name).trim();
  if (INDUSTRY_JA[k]) return INDUSTRY_JA[k];
  // partial / case-insensitive
  const lower = k.toLowerCase();
  for (const [en, ja] of Object.entries(INDUSTRY_JA)) {
    if (en.toLowerCase() === lower) return ja;
  }
  return k; // 未登録は英語のまま（後で追加）
}

function strengthClass(label) {
  if (label === "強い") return "green";
  if (label === "普通") return "yellow";
  return "red";
}
...
function computeIndustryScores(stocks, minCount = 1) {
  ...
  for (const [name, rows] of Object.entries(groups)) {
    const score = Math.round(
      rows.reduce((a, s) => a + (Number(s.rs) || 0), 0) / rows.length
    );
...
function sortedStocks(stocks) {
  const sortBy = (document.getElementById("sortBy") || {}).value || "rs";
  const list = [...(stocks || [])];
  list.sort((a, b) => {
    if (sortBy === "rs") return (b.rs || 0) - (a.rs || 0);
    if (sortBy === "breakout") return (b.breakout_pct || 0) - (a.breakout_pct || 0);
    if (sortBy === "pct30") return (b.pct_from_30w || 0) - (a.pct_from_30w || 0);
    if (sortBy === "indScore") {
...
function renderTable(stocks) {
  ...
  if (!list.length) {
    body.innerHTML = `<tr><td colspan="8" class="empty">Entry銘柄がありません。データ更新を確認してください。</td></tr>`;
    return;
  }
  body.innerHTML = list
    .map((s) => {
      ...
      const indHtml = ind
        ? `<span class="rs ${ind.score >= 80 ? "high" : ind.score >= 55 ? "mid" : "low"}">${ind.score}</span>`
        : "—";
      const bp = s.breakout_pct != null ? Number(s.breakout_pct).toFixed(1) + "%" : "—";
      const p30 =
        s.pct_from_30w != null
          ? (Number(s.pct_from_30w) > 0 ? "+" : "") + Number(s.pct_from_30w).toFixed(1) + "%"
          : "—";
      const price = s.price != null ? Number(s.price).toLocaleString("en-US", { maximumFractionDigits: 2 }) : "—";
      const status = s.status || (s.stage2_entry ? "Entry" : "—");
      return `
      <tr>
        <td>
...
function render() {
  ...
  if (emptyHint) {
    emptyHint.hidden = stocks.length > 0;
    if (!stocks.length) {
...
async function loadData() {
  try {
    ...
    if (el) el.textContent = "読み込み失敗: " + e.message;
  }
}
...
document.getElementById("tvCopyBtn").addEventListener("click", copyTvList);
loadData();
